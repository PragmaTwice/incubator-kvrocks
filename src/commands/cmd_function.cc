...
struct CommandFunction : Commander {
  Status Execute(Server *svr, Connection *conn, std::string *output) override {
    ...
    if (parser.EatEqICase("load")) {
      ...
      std::string libname;
      auto s = Lua::functionLoad(svr, GET_OR_RET(parser.TakeStr()), true, replace, &libname);
      if (!s) return s;
      ...
      *output = SimpleString(libname);
      ...
      return Lua::functionList(svr, libname, output);
    } else if (parser.EatEqICase("delete")) {
      auto s = Lua::functionDelete(svr, GET_OR_RET(parser.TakeStr()));
      if (!s) return s;
...
struct CommandFCall : Commander {
  Status Execute(Server *svr, Connection *conn, std::string *output) override {
    ...
    return Lua::functionCall(svr, args_[1], std::vector<std::string>(args_.begin() + 3, args_.begin() + 3 + numkeys),
